...
**Safety**: Full validation, deploys to all configured hosts simultaneously
...
Affected services:
  • myservice.service (will restart)
  • other-service.service (no restart needed)
...
✗ Critical service failed: myservice.service
...
Check: journalctl -u myservice -n 50
```
...
📝 Changes Applied:
  ...
  • Services Affected: myservice, other-service
  • Services Restarted: myservice (0.8s)
...
⚠️  Warnings:
  1. Service memory usage at 85% (850MB/1GB)
...
```
# 1. Create issue
/new_task "Add new monitoring service"
...
/nix-deploy
Deploy to my-host for issue #123

# 4. Auto-commit and PR
git commit -m "feat(monitoring): add monitoring service (#123)"
gh pr create --fill
```
...
1. **Service Failed to Start**
   ```
   ✗ myservice.service failed

   Automatic Actions:
   1. Captured logs: journalctl -u myservice -n 50
...
### Build Caching
- ✅ Uses binary cache if configured
...
- Which host (your configured hostname, or "all")
