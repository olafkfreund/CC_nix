...
- 📦 Guides through complete packaging workflow
- 🔍 Automatic dependency detection
- 🔐 Hash calculation and verification
- 🏗️ Build phase customization
- 🧪 Testing and validation
- ⏱️ Complete in ~10-15 minutes

```
/new_pkg
Package mycli from GitHub
```

**`/new_vm`** - Create NixOS virtual machines

- 🖥️ Interactive VM configuration
- ⚙️ Custom resource allocation (CPU, memory, disk)
- 🌐 Network configuration (internal/external)
- 🛠️ Service and package setup
- 🚀 Ready-to-run QEMU VMs
- ⏱️ Complete in ~15-20 minutes
...
Specialized knowledge domains for specific NixOS workflows:

### home-manager

Expert guidance for Home Manager configuration and user environment management.

- **Use when**: Configuring user packages, dotfiles, services, and per-user settings
- **Covers**: 50+ program configurations, service management, file management, desktop integration
- **Examples**: Program configs, XDG management, systemd user services, theme integration

### devenv

Fast, declarative development environment setup using devenv.sh.

- **Use when**: Creating project dev environments, language setup, service management
- **Covers**: 55+ languages, 30+ services, process management, containers, pre-commit hooks
- **Examples**: Python/Node/Rust/Go setups, PostgreSQL/Redis services, development shells

### node2nix

Convert NPM packages to Nix expressions for reproducible Node.js development.

- **Use when**: Packaging Node.js apps, managing npm dependencies with Nix
- **Covers**: Lock file support (package-lock.json, npm-shrinkwrap.json), private registries, native modules, overrides
- **Examples**: Converting npm projects, handling native dependencies, monorepo support

### uv2nix

Convert Python/uv projects to Nix expressions using pyproject.nix.

- **Use when**: Packaging Python apps, managing Python dependencies with Nix
- **Covers**: Workspace support, PEP-621, flake integration, NixOS services, development shells
- **Examples**: Converting uv projects, workspace handling, building applications, NixOS integration

### stylix

System-wide theming framework applying color schemes, wallpapers, and fonts across applications.

- **Use when**: Applying consistent themes across NixOS/Home Manager applications
- **Covers**: Base16 color schemes, automatic color generation from wallpapers, font management, multi-platform support
- **Examples**: Theme configuration, wallpaper handling, per-application theming, GNOME/KDE/Hyprland integration

### agenix

Age-encrypted secrets management using SSH keys for secure storage and deployment of sensitive information.

- **Use when**: Managing sensitive data, passwords, API keys, certificates, SSH keys, TLS certificates in NixOS
...
Specialized AI agents for automated NixOS workflows:

### nix-check

Automated linting and checking of NixOS configurations using best-in-class tools.

- **Automatically runs on**: File changes, pre-commit, code review requests
- **Tools**: deadnix (dead code detection), statix (anti-pattern linting), nixpkgs-fmt (formatting)
- **Features**: Comprehensive checking, auto-fix with backups, CI/CD integration, custom rule sets
- **Examples**: Full validation workflow, auto-fix scripts, pre-commit hooks, GitHub Actions

### issue-checker

Monitors NixOS/nixpkgs GitHub issues to identify bugs affecting your installed packages before updates.

- **Automatically runs on**: Before system updates, update planning, CI/CD pipelines
- **Purpose**: Prevent breaking updates, maintain system stability, proactive issue awareness
- **Features**: Package inventory, GitHub API integration, severity classification, actionable recommendations
...
- **Skills**: Reference skill files for deep domain knowledge
- **Subagents**: Let automated agents handle tedious validation tasks
